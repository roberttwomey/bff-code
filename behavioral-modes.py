...
def update_last_command_time():
    """Update the timestamp of the last command sent to the robot."""
    global last_command_time
    last_command_time = time.time()
    print(f"Command sent at {time.strftime('%H:%M:%S')}")

def check_idle_state(conn):
    """Check if the robot has been idle for too long and send StandDown if needed."""
    global last_command_time, is_idle_monitoring
    
    if not is_idle_monitoring:
        return
        
    current_time = time.time()
    idle_duration = current_time - last_command_time
    
    if idle_duration >= IDLE_TIMEOUT:
        print(f"Robot has been idle for {idle_duration:.1f} seconds. Sending StandDown command...")
        asyncio.create_task(send_standdown_command(conn))
        # Update the command time to prevent repeated StandDown commands
        update_last_command_time()

async def send_standdown_command(conn):
    """Send StandDown command to put the robot in prone position."""
    try:
        ...
        print("StandDown command sent successfully")
    except Exception as e:
        print(f"Error sending StandDown command: {e}")

async def idle_monitor(conn):
    """Background task to monitor robot idle state."""
    global is_idle_monitoring
    
    print("Idle monitoring started - will put robot in prone position after 30 seconds of inactivity")
    
    while is_idle_monitoring:
        try:
            check_idle_state(conn)
            await asyncio.sleep(1)  # Check every second
        except Exception as e:
            print(f"Error in idle monitor: {e}")
            await asyncio.sleep(1)

async def send_command_with_tracking(conn, topic, command_data):
    """Send a command to the robot and update the last command time."""
    try:
        await conn.datachannel.pub_sub.publish_request_new(topic, command_data)
        update_last_command_time()
        return True
    except Exception as e:
...
async def main():
    ...
    try:
        ...
        idle_monitor_task = asyncio.create_task(idle_monitor(conn))
        ...
        print("Program running with idle monitoring active...")
        ...
        try:
            while True:
                current_time = time.time()
                idle_duration = current_time - last_command_time
                print(f"Idle time: {idle_duration:.1f}s (StandDown at {IDLE_TIMEOUT}s)")
                await asyncio.sleep(5)  # Update status every 5 seconds
        except KeyboardInterrupt:
            print("\nStopping idle monitoring...")
            is_idle_monitoring = False
        ...
        # Ensure idle monitoring is stopped
        is_idle_monitoring = False
        if 'idle_monitor_task' in locals():
            idle_monitor_task.cancel()
