...
class FidgetBehavior(enum.Enum):
    TURN_LEFT = "turn_left"
...
class BehavioralStateMachine:
    ...
    # Fidget behavior methods
    def fidget_turn_left(self):
        ...
        try:
            ...
            print(f"Error in fidget turn left: {e}")
    
    def fidget_turn_right(self):
        ...
        try:
            ...
            print(f"Error in fidget turn right: {e}")
    
    def fidget_step_forward(self):
        """Small step forward"""
        print("\n[FIDGET] Stepping forward...")
        try:
            # Small step: vx=0.15 m/s, vy=0, vyaw=0 for 0.8 seconds
            self.sport_client.Move(0.25, 0.0, 0.0)
            time.sleep(random.uniform(2.0, 4.0))
            self.sport_client.StopMove()
            self.balance_stand()
        except Exception as e:
            print(f"Error in fidget step forward: {e}")
    
    def fidget_step_back(self):
        """Small step backward"""
        print("\n[FIDGET] Stepping backward...")
        try:
            # Small step: vx=-0.15 m/s, vy=0, vyaw=0 for 0.8 seconds
            self.sport_client.Move(-0.25, 0.0, 0.0)
            time.sleep(random.uniform(2.0, 4.0))
    ...
    def execute_random_fidget(self):
        """Execute a random fidget behavior"""
        if not self.fidget_enabled or self.currently_fidgeting:
            return
        
        # Only fidget in WALKING_MODE
        if self.current_state != RobotState.WALKING_MODE:
            return
        
        # Check if enough time has passed and robot is idle
        current_time = time.time()
        idle_duration = min(current_time - self.last_controller_activity_time, 
                          current_time - self.last_uwb_activity_time)
        
        # Only fidget if idle for at least fidget_idle_threshold seconds
        if idle_duration < self.fidget_idle_threshold:
            return
        
        # Check if it's time for next fidget
        if current_time < self.next_fidget_time:
            return
        ...
        try:
            ...
            print(f"\nError executing fidget {behavior.value}: {e}")
        
        # Schedule next fidget
        self.last_fidget_time = time.time()
        self.next_fidget_time = self.last_fidget_time + random.uniform(
            self.min_fidget_interval, self.max_fidget_interval
        )
        self.currently_fidgeting = False
    ...
    def run_state_machine(self):
        ...
        try:
            while self.monitoring:
                ...
                # Execute random fidget behaviors
                self.execute_random_fidget()
                
                # Display current state
                idle_duration = min(time.time() - self.last_controller_activity_time, time.time() - self.last_uwb_activity_time)
                next_fidget_in = max(0, self.next_fidget_time - time.time())
                fidget_status = "[FIDGETING]" if self.currently_fidgeting else f"[Next fidget: {next_fidget_in:.1f}s]"
                print(f"\rState: {self.current_state.value.upper():15} | Idle: {idle_duration:.1f}s | {fidget_status:30}", end="")
